...
- **Database**: prefer indexed, filtered, paged queries over loading full tables into memory; watch `Meridian.Infrastructure/Repositories` and `AppDbContext` for N+1 query patterns (e.g. lazy-loading `Location` off every `HeatReading`). **Tracking is off by default** — `UseQueryTrackingBehavior(QueryTrackingBehavior.NoTracking)` is set in `Meridian.Infrastructure/DependencyInjection.cs`, so writes must go through `Add`/`Update`/`Remove` explicitly; loading an entity, mutating it and calling `SaveChanges` will *not* persist (call `.AsTracking()` if you ever need that). Any per-location limit belongs in SQL, not in a `.Take()` after materialising — see `GetTemperaturesForLocationsAsync` (LATERAL) and `GetBucketedHistoryAsync` (`DISTINCT ON` + `date_bin`). Add EF Core migrations for any new index rather than relying on default PK-only indexing; there are two: `(LocationId, MeasuredAt DESC)` for per-location lookups and `(MeasuredAt DESC)` for pure time-range filters (history window, retention sweep).
- **Backend API**: responses are Brotli/Gzip compressed (`UseResponseCompression`), which matters most for `/api/heat/history`; keep hot paths (dashboard, heat ingestion, SignalR broadcast) allocation-light and async all the way down; reuse the existing `HttpClient` + Polly retry pattern (`DependencyInjection.cs`) rather than instantiating new clients; be deliberate about what `LiveHeatSimulatorService`'s tick and `HeatIngestionWorker`'s 15-min poll write to Postgres, since both feed the same hot table.
- **Frontend**: keep React Query as the cache of record and avoid triggering extra fetches outside it (see the `CommandPalette` `enabled: open` fix — its dashboard query used to fire on every route); keep heavy client-only libraries behind `dynamic(..., { ssr: false })` — Leaflet via `components/Map`, and Recharts via `components/features/charts/*` (importing `recharts` directly into a route page puts ~110 kB back into its first load); avoid adding dependencies that aren't imported anywhere — audit with a grep before adding new UI/animation/form libraries.
...
- **Two independent background services both write `HeatReading` rows** — know which one you're touching before changing ingestion logic. Both are bounded by `DataRetentionService` (`Meridian.API/Services`), which trims raw readings past `Retention:RawReadingDays` (default 7) every `Retention:SweepIntervalHours` (default 6), in batches, always preserving each location's most recent reading:
  ...
  - `LiveHeatSimulatorService` (`Meridian.API/Services`) — mutates a random location's last reading by ±1.5°C to keep the live dashboard visually active, then pushes it over SignalR. This is a demo/UI-liveliness feature, not real telemetry. It is **config-gated**: `Simulator:Enabled` (default `true`) decides whether it is registered at all, and `Simulator:IntervalSeconds` (default 2.5) sets the tick. Set `Simulator__Enabled=false` in any environment where the data matters — it appends to the same table as real ingestion.
...
- **Controllers** (`Meridian.API/Controllers`, all `api/[controller]`): `HeatController` (readings, dashboard — 10s `IMemoryCache`, ingest, and history — downsampled server-side to a fixed 60 frames whatever window is asked for), `LocationController` (CRUD + bulk CSV-style ingestion), `AnalysisController` (correlations/trends), `AgentController` (`/query`, `/stream` — the Groq agent), `ReportController` (generate/list/delete), `ExportController` (`/excel` via ClosedXML, `/pdf` via QuestPDF + ScottPlot charts, see `Meridian.API/Exports`).
...
- DB migrations run automatically on API startup via `DatabaseInitializer.MigrateAsync` (`Meridian.API/Startup`) — no separate migration step needed for local dev once the connection string is set. It retries 5 times with exponential backoff so a transient outage doesn't become a restart loop, and fails fast with an explicit message if the connection string is still the `appsettings.json` placeholder.
